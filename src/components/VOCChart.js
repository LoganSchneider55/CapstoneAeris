...
export default function VOCChart({ data = [], latest }) {
  const chartRef = useRef(null);
  if (!data.length) return null;

  // VOC thresholds (TVOC in ppb)
 const thresholds = [
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        intersect: false,
        callbacks: {
          label: (ctx) => `VOC: ${ctx.parsed.y} ppb`,
        },
      },
      annotation: {
  ...
  return (
    ...
    >
      <h3 style={{ marginBottom: 10 }}>
        VOC Levels — <span style={{ color: activeLevel.color }}>{activeLevel.label}</span>
      </h3>
